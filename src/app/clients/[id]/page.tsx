...
export default function ClientDetailPage() {
  ...
  useEffect(() => {
    if (!orgId) return;
    fetch('/api/ai/lifecycle')
      .then((res) => (res.ok ? res.json() : Promise.reject('Failed')))
      .then((data: Array<{ org_id: string; stage: string; confidence: number; signals: string[]; next_stage_prediction: string | null; alert: string | null }>) => {
        const match = Array.isArray(data) ? data.find((d) => d.org_id === orgId) : null;
        if (match) setLifecycleData(match);
      })
      .catch(() => {});
  }, [orgId]);
              ...
              <div className="bg-card rounded-xl border border-border p-5 space-y-4">
                ...
                {lifecycleData && (() => {
                  ...
                  const stageColors: Record<string, string> = {
                    prospecting: 'bg-primary/20 text-primary',
                    onboarding: 'bg-violet-500/20 text-violet-400',
                    building_trust: 'bg-success/20 text-success',
                    deep_work: 'bg-success/20 text-success',
                    sustaining: 'bg-primary/20 text-primary',
                    winding_down: 'bg-warning/20 text-warning',
                    at_risk: 'bg-danger/20 text-danger',
                  };
                  const stageOrder = ['prospecting', 'onboarding', 'building_trust', 'deep_work', 'sustaining', 'winding_down', 'at_risk'];
                  const currentIdx = stageOrder.indexOf(lifecycleData.stage);

                  return (
                    <div>
                      <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 flex items-center gap-1">
                        <Activity className="w-3.5 h-3.5" /> Engagement Lifecycle
                      </h4>
                      {/* Stage pipeline */}
                      <div className="flex items-center gap-1 mb-3">
                        {stageOrder.filter(s => s !== 'at_risk').map((stage, i) => (
                          <div key={stage} className="flex items-center gap-1 flex-1">
                            <div
                              className={cn(
                                'h-1.5 rounded-full flex-1 transition-all',
                                i <= currentIdx && lifecycleData.stage !== 'at_risk'
                                  ? 'bg-primary'
                                  : 'bg-border'
                              )}
                            />
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <span className={cn(
                          ...
                          stageColors[lifecycleData.stage] || 'bg-muted/20 text-muted'
                        )}>
                          {stageLabels[lifecycleData.stage] || lifecycleData.stage}
                        </span>
                        <span className="text-[10px] text-muted">
                          {Math.round(lifecycleData.confidence * 100)}% confidence
                        </span>
                        {lifecycleData.next_stage_prediction && (
                          <span className="text-[10px] text-muted/60 ml-auto">
                            Next: {stageLabels[lifecycleData.next_stage_prediction] || lifecycleData.next_stage_prediction}
                          </span>
                        )}
                      </div>
                      {lifecycleData.alert && (
                        <p className="text-xs text-warning/80 flex items-center gap-1 mb-2">
                          <AlertTriangle className="w-3 h-3 flex-shrink-0" />
                          {lifecycleData.alert}
                        </p>
                      )}
                      {lifecycleData.signals.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {lifecycleData.signals.slice(0, 4).map((signal, i) => (
                            <span key={i} className="text-[10px] px-2 py-0.5 rounded bg-card-hover text-muted">
                              {signal}
                            </span>
                          ))}
                        </div>
                ...
                {/* Quick Coaching Signals from trajectory */}
                {trajectoryData && (() => {
                  const momentum = trajectoryData.momentum as { direction?: string; reasoning?: string } | undefined;
                  const stalledAreas = trajectoryData.stalled_areas as Array<{ topic?: string }> | undefined;
                  const droppedThreads = trajectoryData.dropped_threads as Array<{ item?: string; significance?: string }> | undefined;
                  const highPriorityDropped = (droppedThreads || []).filter(t => t.significance === 'high');

                  return (
                    <div className="border-t border-border/50 pt-3">
                      <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 flex items-center gap-1">
                        <Sparkles className="w-3.5 h-3.5" /> Coaching Signals
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        {momentum?.direction && (
                          <div className={cn(
                            ...
                            momentum.direction === 'accelerating' ? 'bg-success/10' :
                            momentum.direction === 'steady' ? 'bg-primary/10' :
                            momentum.direction === 'stalling' ? 'bg-warning/10' : 'bg-danger/10'
                          )}>
                            <p className="text-[10px] text-muted uppercase tracking-wider mb-0.5">Momentum</p>
                            <p className={cn(
                              'text-sm font-semibold capitalize',
                              momentum.direction === 'accelerating' ? 'text-success' :
                              momentum.direction === 'steady' ? 'text-primary' :
                              momentum.direction === 'stalling' ? 'text-warning' : 'text-danger'
                            )}>
                              {momentum.direction}
                            </p>
                          </div>
                        ...
                        {stalledAreas && stalledAreas.length > 0 && (
                          <div className="bg-warning/10 rounded-lg px-3 py-2">
                            <p className="text-[10px] text-muted uppercase tracking-wider mb-0.5">Stalled Areas</p>
                            <p className="text-sm font-semibold text-warning">{stalledAreas.length} topic{stalledAreas.length !== 1 ? 's' : ''}</p>
                          </div>
                        )}
                        {/* Dropped high-priority */}
                        {highPriorityDropped.length > 0 && (
                          <div className="bg-danger/10 rounded-lg px-3 py-2">
                            <p className="text-[10px] text-muted uppercase tracking-wider mb-0.5">Dropped Threads</p>
                            <p className="text-sm font-semibold text-danger">{highPriorityDropped.length} high priority</p>
                          </div>
                        )}
                      </div>
                    </div>
